...
datDIF <- data[ QCF_GLB == "good" & !is.na(wattDIF) ]
data   <- datDIF
...
for (av in c(wecare, "relN")) {
    ...
    print(
        levelplot(mtx,
                  ...
                  main = paste("Diffuse Irradiance asymetry:", av),
                  col.regions = Ocols,
                  at = brk,
                  panel=function(...) {
                      arg <- list(...)
...
print(
    levelplot(mtx,
              ...
              xlab = "Declination from South",
              ylab.right = "West                                 East",
              main = paste("Diffuse Irradiance asymetry:", av, "p <", p_lim, "relN <", rel_lim ),
              col.regions = Ocols,
              at = brk,
              panel=function(...) {
                  arg <- list(...)
...
## same scale
max_abs <- max(abs(sky_diff_m[[av]]),na.rm = T)
print(paste("Max scale:", max_abs))
# 120
max_abs <- 120
brk     <- do.breaks(c(-max_abs, max_abs), breaks)
for (im in 1:12) {
    ...
    print(
        levelplot(mtx,
                  ...
                  xlab = "Declination from South",
                  ylab.right = "West                                 East",
                  main = paste("Diffuse Irradiance asymetry:", av, "Month:",im),
                  col.regions = Ocols,
                  at = brk,
                  panel=function(...) {
                      arg <- list(...)
...
#                   xlab = "Declination from South",
#                   ylab.right = "West                                 East",
#                   main = paste("Difuse Irradiance Ratio:", av),
#                   col.regions = Ocols,
#                   at = brk,
#                   panel=function(...) {
#                       arg <- list(...)
